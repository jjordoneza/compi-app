...
### Auditoría técnica completa + 7 bugs de la sección F del checklist — 17 jul 2026
Antes de la auditoría manual del 17 jul se hizo una pasada completa de documentos-vs-código: ver **`docs/auditoria-tecnica-completa.md`** (19 secciones, etiquetadas `[SIN CAMBIOS]`/`[MODIFICADO]`/`[NUEVO]`, con sección de brechas inversas al final — el hallazgo más importante es el cambio de fórmula del IDC, sección 15.1, y la brecha más grande sigue siendo el agente de WhatsApp, sección 6). De ahí salió `docs/checklistauditoria.md` (checklist de pruebas) y, leyendo el código para armarlo, se encontraron y corrigieron 7 bugs reales sin esperar a las pruebas manuales:

1. Onboarding de proveedores: spinner infinito si fallaba la carga, sin manejo de error.
2. admin-web Proveedores/Productos nuevos: orden de ternario dejaba "Cargando..." infinito aunque hubiera error.
3. Maestro proveedores/productos (admin-web): "Cancelar" no revertía los campos editados.
4. Envío de abastecimiento: inserts sueltos sin transacción — reintento tras fallo parcial duplicaba. Nueva RPC `crear_abastecimiento` (migración 0028) atómica.
5. Pegar pedido / Importar contactos: mismo riesgo de duplicados en reintento — cada ítem se marca al guardarse.
6. Mi negocio (tendero): `barrio` vacío se guardaba como `''` en vez de `null`.
7. Splash: sin timeout, spinner de arranque podía quedar colgado para siempre.

También se corrigió por separado (antes de esta ronda) un bug de producción real en `aprobar_cambio_comercio` (migración 0023 referenciaba una columna eliminada en 0014) — migración 0027, PR #40, ya mergeado.

**Pendiente de tu lado:** aplicar la migración `0028_crear_abastecimiento_atomico.sql` en el SQL Editor de Supabase antes de probar "Enviar abastecimiento" en el checklist.
