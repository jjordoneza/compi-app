...
**Bloque 3:** GPS con mapa de confirmación al registrar negocio — `react-native-maps` 1.20.1 + plugin en `app.json` (placeholder de API key de Google Maps para Android, **falta reemplazar antes de buildear**), nueva pantalla `ConfirmarUbicacionScreen` con pin arrastrable. **No probado en dispositivo** — requiere build de EAS `development` (Expo Go no sirve con este módulo nativo + `newArchEnabled`).
