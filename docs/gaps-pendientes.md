...
**Pendiente de decidir con el usuario (P3, no implementado todavía):** "Ver historial de un negocio eliminado" — hoy `MiNegocioTenderoScreen` promete en el diálogo de eliminar que "tu historial de pedidos queda intacto por si lo necesitas después", pero no hay ningún camino en la app para llegar a verlo: `MisComercios.listar()` filtra por `activo=true`, así que un negocio desactivado desaparece de `SeleccionarNegocioScreen`, y si era el único negocio del usuario, `SplashScreen` lo manda directo a `RegistroNegocio` (nunca pasa por Perfil/Seleccionar). Propuesta pendiente de confirmar: (1) `SplashScreen` con 0 comercios activos debería revisar si el usuario tiene comercios *inactivos* antes de mandarlo a registro — si los tiene, llevarlo a una versión de `SeleccionarNegocioScreen` que también liste "Negocios eliminados"; (2) esa pantalla necesita una nueva vista de solo-lectura (lista de abastecimientos pasados) por negocio eliminado, en vez de reabrir todo el Home (evita tener que volver de solo-lectura toda la app). No implementado — requiere confirmar el diseño con el usuario primero.
